...
export default function ProfileViewer() {
  ...
  const { getAnotherUser, getUserFollowData, getAnotherInventory } =
    useAuthorization();
          ...
          <div className="container-profile">
            <div className="container-profile-data">
              <div className="full-width d-flex justify-content-center align-items-center p-5 flex-wrap">
                ...
                <div>
                  <h3 style={{ color: "#4a5659" }}>
                    {anotherUserData.firstname} {anotherUserData.lastname}
                  </h3>
                  <div className="d-flex">
                    <p style={{ color: "#86979c", fontSize: "26px" }}>
                      @{anotherUserData.username}
                    </p>
                    <div
                      className="d-flex justify-content-center align-items-center ms-3 pointer"
                      onClick={() =>
                        history.push(
                          `/app/chat?duo_id=${anotherUserData._id}&duo_username=${anotherUserData.username}`
                        )
                      }
                      style={{
              ...
              <div className="full-width d-flex justify-content-center align-items-center px-5 mb-3 flex-wrap">
                <div className="d-flex justify-content-center align-items-center followers mx-2 px-5 py-3 my-2">
                  <b className="mx-2 number">{followData.follower.length} </b>{" "}
                  followers
                </div>
                <div className="d-flex justify-content-center align-items-center following mx-2 px-5 py-3 my-2">
                  <b className="mx-2 number">{followData.following.length} </b>{" "}
                  following
                </div>
              </div>
            </div>
            ...
            <div className="full-width">
              <div>
                <div className="d-flex flex-wrap">
                  {inventory_item.length <= 10
                    ? inventory_item
                    : inventory_item.slice(0, 10)}
                </div>
              </div>
            </div>
          </div>
