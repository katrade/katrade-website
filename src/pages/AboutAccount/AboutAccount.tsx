...
function AboutAccount(userData: any) {
  ...
  function SelectComp(c: any) {
    if (destCompState.dest === "Account" || c === "account") {
      ...
      destCompState.dest === "ChangePassword" ||
      c === "changePassword"
    ) {
      ...
      setComponentPage(
        <HistoryComp data={historyData} checkUser={accountData} />
      );
    }
  }
  ...
  return (
    <DestCompContext.Provider value={{ destCompState, destCompDispatch }}>
      <div>
        <Background>
          <Navbar
            image={accountData.profilePic}
            handleComponent={(c: any) => SelectComp(c)}
          />
          <Block height="50">
            <div>
              <div>
                ...
                <div className="d-flex">
                  ...
                  <div style={{ width: "100%" }}>{componentPage}</div>
                </div>
              </div>
            </div>
          </Block>
