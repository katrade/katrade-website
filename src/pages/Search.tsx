...
export default function Search() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (
        params.quote.split("-")[params.quote.split("-").length - 1] ==
        "byCategory"
      ) {
        var getSrch: any = await searchByCategory(
          params.quote.split("-")[0],
          params.quote.split("-")[1]
        );
        setSearchData(getSrch);
      } else if (
        params.quote.split("-")[params.quote.split("-").length - 1] == "byText"
      ) {
        var getSrch: any = await getSearch(
          params.quote.split("-").slice(0, -1).join("")
        );
        setSearchData(getSrch);
  ...
  if (
    ...
    return (
      <Background>
        ...
        >
          <div className="my-4">
            <H5 className="mb-3">
              Search "{params.quote.split("-").slice(0, -1).join("")}".
            </H5>
            <div className="d-flex flex-wrap">{rec_item}</div>
            <div
              className={
                found == 0
    ...
    searchData &&
    params.quote.split("-")[params.quote.split("-").length - 1] == "byCategory"
  ) {
    const rec_item = searchData.map((item: any, index: any) => {
      ...
      <Background>
        ...
        >
          <div className="my-4">
            <H5 className="mb-3">
              Search "
              {params.quote.split("-").slice(0, -1)[1] == "none"
                ? params.quote.split("-").slice(0, -1)[0]
                : params.quote.split("-").slice(0, -1).join(": ")}
              ".
            </H5>
    ...
    searchData &&
    (params.quote == "match-byseemore" || params.quote == "favorite-byseemore")
  ) {
    const rec_item = searchData.map((item: any, index: any) => {
      found += 1;
