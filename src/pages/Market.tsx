...
function Market() {
  ...
  const tmpMatchInventory =
    matchInventory === null
      ? []
      : matchInventory.map((item: any, index: any) => {
          if (item.owner != localStorage.getItem("uid")) {
            return (
              <Recommend item={item.match} match={item.matchWith} key={index} />
            );
          }
        });
  const tmpInventory =
    allInventory === null
      ? []
      : allInventory.map((item: any, index: any) => {
          if (item.owner != localStorage.getItem("uid")) {
                  ...
                  {matchInventory !== null ? (
                    tmpMatchInventory.length == 0 ? (
                      <div className="mx-auto my-5 text-center">
                        <p className="notfounditem">
                          ไม่พบสิ่งของที่มีความต้องการตรงกับคุณ
                        </p>
                      </div>
                    ) : tmpMatchInventory.length <= 10 ? (
                      tmpMatchInventory
                    ) : (
                      tmpMatchInventory.slice(0, 10)
                    )
                  ) : (
                ...
                {tmpMatchInventory.length > 10 ? (
                  <div className="d-flex justify-content-center align-items-center my-3">
                    <div
                      className="seemore"
                      onClick={() =>
                        history.push(`/app/search/match-byseemore`)
                      }
                    >
                      <p className="text-white font-weight-bold">see more</p>
                    </div>
                  </div>
                ) : (
                  <></>
              ...
              <div className="d-flex justify-content-start flex-wrap">
                {allInventory !== null ? (
                  tmpInventory.length == 0 ? (
                    <div className="mx-auto my-5 text-center">
                      <p className="notfounditem">
                        ไม่พบสิ่งของบน katrade marlet เลย
                      </p>
                    </div>
                  ) : tmpInventory.length > 20 ? (
                    tmpInventory.slice(0, 20)
                  ) : (
                    tmpInventory
                  )
                ) : (
              ...
              {tmpInventory.length > 20 ? (
                <div className="d-flex justify-content-center align-items-center my-3">
                  <div
                    className="seemore"
                    onClick={() =>
                      history.push(`/app/search/favorite-byseemore`)
                    }
                  >
                    <p className="text-white font-weight-bold">see more</p>
                  </div>
