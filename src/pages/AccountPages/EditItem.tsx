...
export default function EditItem() {
  ...
  SubCategoriesEn.push([
    {
      value: "Please select a main category",
      label: "Please select a main category",
    },
  ]);
  SubCategoriesTh.push([
    {
      value: "กรุณาเลือกหมวดหมู่หลักก่อน",
      label: "กรุณาเลือกหมวดหมู่หลักก่อน",
    },
  ]);
  ...
  useEffect(() => {
    ...
    if (productDetail._id && MainCategoriesEn && SubCategoriesEn) {
      const main = MainCategoriesEn.filter(
        (e: any) => e.value === productDetail.category.parentCategoryEn
      );
      setMainCat({
        ...MainCat,
        Mvalue: productDetail.category.parentCategoryEn,
        MindexC: main[0].indexC,
      });
      hide();
    }
  }, [productDetail]);
      ...
      if (mySubCateIndex == -1) {
        return (
          <Select
            ...
            className="fs-5"
            name="mySubCate"
            placeholder="กรุณาเลือก Category หลักก่อน"
          />
        );
      } else {
        return (
          <Select
            ...
            defaultValue={{
              value: productDetail.category.childCategoryEn,
              label: `${productDetail.category.childCategoryEn}`,
            }}
          />
        );
      }
  ...
  function subcate() {
    ...
    return (
      <Select
        options={SubCategoriesEn[mySubCateIndex + 1]}
        className="fs-5"
        name="mySubCate"
        onChange={(sel: any) => {
          selectMySubTh(sel);
  ...
  function selectMySub(selectMySubCate: any) {
    ...
    setProductDetail({
      ...productDetail,
      category: { ...productDetail.category, parentCategoryEn: mySubCateTag },
    });
  }
  function selectMySubTh(event: any) {
    ...
    setProductDetail({
      ...productDetail,
      category: { ...productDetail.category, childCategoryEn: event.value },
    });
  }
  ...
  useEffect(() => {
    if (
      ...
      for (let index = 0; index < wantInputFields.length; index++) {
        const mainreq = MainCategoriesEn.filter(
          (e: any) => e.value === value[index]["reqCat"]["parentCategoryEn"]
        );
        ind.push(mainreq[0]);
      }
  ...
  const [godHelpMe0, setGodHelpMe0] = useState<any>(
    <Select
      options={SubCategoriesEn[0]}
      className="fs-5"
      name="wantSubCate"
      placeholder="กรุณาเลือก Category หลักก่อน"
    />
  );
  const [godHelpMe1, setGodHelpMe1] = useState<any>(
    <Select
      options={SubCategoriesEn[0]}
      className="fs-5"
      name="wantSubCate"
  ...
  const [godHelpMe2, setGodHelpMe2] = useState<any>(
    <Select
      options={SubCategoriesEn[0]}
      className="fs-5"
      name="wantSubCate"
  ...
  const [godHelpMe3, setGodHelpMe3] = useState<any>(
    <Select
      options={SubCategoriesEn[0]}
      className="fs-5"
      name="wantSubCate"
  ...
  const [godHelpMe4, setGodHelpMe4] = useState<any>(
    <Select
      options={SubCategoriesEn[0]}
      className="fs-5"
      name="wantSubCate"
  ...
  function changesub(index: any) {
    ...
    if (index == 0) {
      return (
        <Select
          options={SubCategoriesEn[wantSubCateIndex + 1]}
          className="fs-5"
          name="wantSubCate"
          onChange={(event) => {
          ...
          placeholder={placeholderString}
          defaultValue={{
            value: wantInputFields[index]["reqCat"]["childCategoryEn"],
            label: `${wantInputFields[index]["reqCat"]["childCategoryEn"]}`,
          }}
        />
      );
    } else if (index == 1) {
  ...
  function selectWantSub(index: any, event: any) {
    ...
    values[index]["reqCat"]["parentCategoryTh"] =
      MainCateLangTh[wantSubCateIndex];
    ...
    if (index == 0) {
      setGodHelpMe0(
        <Select
          ...
          className="fs-5"
          name="wantSubCate"
          onChange={(event) => selectedSub(index, event)}
          placeholder={placeholderString}
        />
      );
    } else if (index == 1) {
  ...
  function selectedSub(index: any, event: any) {
    ...
    values[index]["reqCat"]["childCategoryTh"] =
      SubCateLangTh[0][MainCatReq0[index].indexC][event.indexC];
    SetWantInputFields(values);
  }
      ...
      <Block height="auto" backgroundColor="#f7fafc">
        <form onSubmit={handleUnload}>
          <div className="p-3 my-4 bg-white">
                ...
                <div className="d-flex justify-content-around flex-wrap">
                  ...
                  <div className="position-relative">
                    ...
                    <ImCross
                      className={
                        dataCover
                          ? "position-absolute rounded-circle bg-secondary pointer"
                          : "d-none"
                      }
                      onClick={() => {
                        setCover("https://via.placeholder.com/120");
                        setDataCover(undefined);
                      }}
                      style={{
                        top: "-7px",
                        right: "-8px",
                  ...
                  <div className="position-relative">
                    ...
                    <ImCross
                      ...
                      onClick={() => {
                        setPicture1("https://via.placeholder.com/120");
                        setDataPicture1(undefined);
                      }}
                      style={{
                        top: "-7px",
                        right: "-8px",
            ...
            <div className="form-group row">
              ...
              <div className="col-md-5">
                <Select
                  ...
                  defaultValue={{
                    value: finalMyMainCate,
                    label: `${productDetail.category.parentCategoryEn}`,
                  }}
                />
              </div>
              <div className="col-md-5">{subcate()}</div>
            </div>
            <div className="form-group row">
                ...
                <div className="form-group row">
                  ...
                  <div className="col-md-5">
                    <Select
                      ...
                      placeholder="กรุณาเลือก Category หลัก"
                      defaultValue={{
                        value: inputField.reqCat.parentCategoryEn,
                        label: `${inputField.reqCat.parentCategoryEn}`,
                      }}
                    />
                  </div>
                  <div className="col-md-5">{changesub(index)}</div>
                </div>
                <div className="form-group row">
                  <label className="col-md-2 col-form-label fs-5">
                    Requirement Details
            ...
            <p className="fs-5 text-mute text-right mt-3 mb-0">
              *เนื่องจากการแสดงผลที่ไม่สมบูรณ์
              จึงขอความร่วมมือผู้ใช้งานค่อยๆทำการเลือกแต่ละรายการอย่างใจเย็น
            </p>

            {/* ปุ่มยืนยัน */}
            <hr className="my-4" />
            <div className="d-flex justify-content-center">
              <SolidButton width="100px" buttonColor="#15C777">
                <input
                  type="submit"
                  value="Submit"
              ...
              <SolidButton width="100px" buttonColor="red">
                <a
                  href="/app/aboutaccount?component=inventory"
