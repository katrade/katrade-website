...
export default function SignupForm({pw, setPw}:p){
    ...
    const checkSame = async () => {
        let result = await fetch(`http://localhost:5000/user/checkUn?username=${form.username}`).then(res => res.json());
    ...
    const onFormSubmit = async () => {
        ...
        let result:any = await fetch('http://localhost:5000/user/signup',{
            method: 'POST',
            mode: 'cors',
            cache: 'no-cache',
