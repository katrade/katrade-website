...
function Recommend(itemData: any, match: any) {
  ...
  const matchlist = matchwith.map((data: any, index: any) => {
    return (
      <div
        className="px-2 rounded-3 ms-2"
        style={{ backgroundColor: "#15C777" }}
      >
  ...
  return (
    <div
      className={theme === "light" ? "recommend-card" : "recommend-card-dark"}
    >
      <div
      ...
      <div className="recommend-text-card" onClick={LinkItemData}>
        ...
        <p className="d-flex desc item-desc text-truncate">
          ตรงกับ : {matchlist}
        </p>
      </div>
      <div
        className="text-end px-2"
        onClick={() => history.push(`/app/profileviewer?user_id=${item.owner}`)}
      >
        <p
          className="hoverUsername"
          style={{ color: "#00b34a", fontSize: "16px" }}
        >
          {item.username}
        </p>
      </div>
