...
export const UserContext = React.createContext<{ valid: boolean | null }>({
  valid: null,
});
...
export function UserProvider({ children }: propsInterface) {
  ...
  const checkValidity = async () => {
    setChecked(true);
    await getUserData().then((u: IAccount | null) => {
      setChecked(true);
      if (u === null) {
        setValid(false);
        hide();
  ...
  return (
    <UserContext.Provider value={{ valid }}>{children}</UserContext.Provider>
  );
}
